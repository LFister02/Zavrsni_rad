...
void MonocularSlamNode::initialPoseCallback(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
{
    ...
    RCLCPP_INFO(this->get_logger(), "Nova početna pozicija primljena: x=%.2f y=%.2f z=%.2f",
...
void MonocularSlamNode::GrabImage(const ImageMsg::SharedPtr msg)
{
    ...
    try
    {
        ...
        // Korekcija nagiba
        Eigen::Matrix3f correction = Eigen::AngleAxisf(-0.1927, Eigen::Vector3f::UnitX()).toRotationMatrix();

        // Rotiraj t_ros
        Eigen::Vector3f t_ros_corrected = correction * t_ros;
        ...
        tf_msg.transform.translation.x = position_(0) + t_ros_corrected(0) * scale;
        tf_msg.transform.translation.y = position_(1) + t_ros_corrected(1) * scale;
        tf_msg.transform.translation.z = 0.0f + t_ros_corrected(2) * scale;
    ...
    sensor_msgs::msg::PointCloud2 MonocularSlamNode::convertMapPointsToPointCloud(const std::vector<ORB_SLAM3::MapPoint*>& mapPoints)
    {
        ...
        Eigen::Matrix3f orb_to_ros;
        orb_to_ros << 1, 0, 0,
                      0, 0, 1,
                      0,-1, 0;

        // Korekcija nagiba kamere - Kamera na pravom dronu je pod blagim nagibom.
        Eigen::Matrix3f correction;
        correction = Eigen::AngleAxisf(-0.1927, Eigen::Vector3f::UnitX()).toRotationMatrix(); 
        Eigen::Matrix3f total_transform = correction * orb_to_ros;
        ...
        for (auto mp : mapPoints)
        {
            ...
            Eigen::Vector3f pos_ros = total_transform * pos_orb;

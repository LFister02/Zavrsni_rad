...
class DynObsReplan(Node):
    def __init__(self):
        super().__init__('dyn_obs_replanning')  
        self.marker_pub = self.create_publisher(Marker, 'visualization_marker', 10)
        self.path_pub = self.create_publisher(Path, '/astar_path', 10)
        self.goal_subscriber = self.create_subscription(PoseStamped, '/goal_pose', self.goal_callback, 10)
        self.new_obs_sub = self.create_subscription(PointStamped, 'new_obstacle', self.mission_replanning, 10)
        ...
        self.start = None
        self.max_z = int(7.0 * self.resolution // self.resolution)
        ...
        self.load_voxels('putanja_do_CSV_datoteke') #   Potrebno definirati
        self.publish_obstacles()
        self.obstacle_buffer_zone(self.occupied_voxels, layers=1)
 

    #   Detekcija dinamičke/nove prepreke
    #   Objava markera na mjestu prepreke i definiranje novih zauzetih ćelija
    def mission_replanning(self, msg):
        self.get_logger().info("Primljena nova prepreka!")
        x = int(float(msg.point.x / self.resolution))
        y = int(float(msg.point.y / self.resolution))
        self.get_logger().info(f"Prepreka je na koordinatama: {x, y}")
        ...
        self.obstacle_buffer_zone(new_voxels, layers=2) #   2 sloja oko nove prepreke za veću sigurnost
    ...
    def obstacle_buffer_zone(self, voxels, layers = 1):

        directions = [(dx, dy, dz) for dx in [-1, 0, 1]
                               for dy in [-1, 0, 1]
                               for dz in [-1, 0, 1]
                               if (dx, dy, dz) != (0, 0, 0)]
        current_layer = set(voxels)

        for _ in range(layers):
            next_layer = set()
            for vx, vy, vz in current_layer:
    ...
    # Računanje troška za prelezak iz trenutnog čvora na susjedni
